...
import { Users, Receipt, ArrowRight, Wallet, ChevronLeft, CreditCard, AlertTriangle } from 'lucide-react';
  ...
  if (error) return (
    <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center p-8">
      <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-8 max-w-lg text-center">
        ...
        <p className="text-gray-400 mt-4 text-sm">Import the CSV first to populate data.</p>
        <Link href="/import" className="mt-4 inline-block px-6 py-2 bg-white text-black rounded-xl font-medium">Go to Importer</Link>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-8 font-sans selection:bg-indigo-500/30 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          ...
          <Link href="/import" className="px-6 py-3 bg-white/10 text-white border border-white/10 hover:bg-white/20 rounded-2xl font-medium transition-all backdrop-blur-md flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-4 h-4 mr-2"><path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242"/><path d="M12 12v9"/><path d="m16 16-4-4-4 4"/></svg>
            Import CSV
          </Link>
        </div>
            ...
            <div className="bg-white/5 p-8 rounded-[2rem] shadow-2xl border border-white/10 backdrop-blur-xl flex-1">
              ...
              {data.balances.length === 0 ? (
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p>No members yet. Import the CSV first.</p>
                </div>
              ) : (
                    ...
                    <div key={i} className="group relative p-5 rounded-2xl bg-black/20 border border-white/5 hover:border-white/20 transition-all overflow-hidden">
                      <div className="flex justify-between items-center relative z-10">
                        ...
                        <div className="text-right">
                          <div className={`text-xl font-bold tracking-tight ${b.net > 0 ? 'text-green-400' : b.net < 0 ? 'text-red-400' : 'text-gray-500'}`}>
              ...
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-4 custom-scrollbar">
                {data.expenses.map((e: any) => (
                  <div key={e.id} className="group p-5 rounded-2xl bg-black/20 border border-white/5 hover:border-white/20 transition-all hover:bg-white/5">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="font-bold text-gray-100 text-lg">{e.description}</h3>
                      ...
                      <span className="font-bold text-xl text-white">{e.amount.toFixed(2)} INR</span>
                    </div>
                ...
                {data.expenses.length === 0 && (
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <p className="text-gray-600 text-sm mt-2">Run the importer to populate expenses.</p>
                    <Link href="/import" className="mt-4 px-5 py-2 bg-indigo-600 text-white rounded-xl text-sm font-medium hover:bg-indigo-500 transition-colors">
                      Go to Importer →
                    </Link>
                  </div>
                )}
